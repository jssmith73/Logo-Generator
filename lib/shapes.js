class Shape {
    ...
    // const triangleSyn = new Triangle(
    //     `<polygon points="150, 18 244, 182 56, 182" fill="${answers.color}" />`
    // )

    // const circleSyn = new Circle(
    //     `<circle cx="150" cy="100" r="80" fill=${answers.color} />`
    // )

    // const squareSyn = new Square(
    //     `<rect x="90" y="40" width="120" height="120" fill="${answers.color}" />`
    // )

    const createLogo = (answers) => {
        let logo = new Shape (answers.text, answers.textColor, answers.shape, answers.shapeColor);
        switch (answers.shape) {
            case 'circle':
                new Circle(`<circle cx="150" cy="100" r="80" fill="${answers.shapeColor}" />`) 
                break
            case 'triangle':
                new Triangle(`<polygon points="150, 18 244, 182 56, 182" fill="${answers.shapeColor}" />`);
                break
            default:
                new Square(`<rect x="90" y="40" width="120" height="120" fill="${answers.shapeColor}" />`);
        };
